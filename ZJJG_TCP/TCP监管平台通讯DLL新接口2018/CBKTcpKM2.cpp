...
BOOL CCBKTcpKM2App::InitInstance() 
{
	...
	logRData.Format("%s\\JGPTRData.log",tempPath);//监管平台返回值保存

	CFileFind FindRDFile;
	if (FindRDFile.FindFile(logRData))
	{
		CFile::Remove(logRData);
	}
	//////////////////////////////////////////////////////////////////////////
	WriteLogOne("InitInstance START CBKTcpKM2 DLL! 20180129 1040");	
	//////////////////////////////////////////////////////////////////////////	
	...
	WriteLogOne("%s,8889 20180129 1040",JGPTIP);
	sendi=1;
	ConnOK=FALSE;
	UINT tempcardnum;
...
// 	m_sendList.AddTail(msendbuf);
	...
	for (int li=0;li<forcount;li++)
	{
		...
		if(GetPrivateProfileString("LISTSTR",linestr,"",getstr.GetBuffer(MAX_PATH),MAX_PATH,".\\CSTRNOTSEND.dll"))
		{
			...
			{
				WriteLogOne("未上传数据不正确,%d=%s",li,getstr);;
				continue;
			}
			m_sendList.AddTail(msendbuf);
		}
...
//	HsendThread=::CreateThread(NULL,0,SendThreadProc,NULL,0,0);
	...
	{ 
		WriteLogOne("加载 CSendDataToJMQ.dll 失败!");
		return FALSE;
	}
...
//	CString strlog;
	...
	::WritePrivateProfileString("JGPT",strHead,NULL,theApp.logRData);
...
extern "C" __declspec( dllexport ) char *  GetRCode(LPTSTR packstr,int packi)
{
	static char rCodeC[260] = { 0 };
	CString strHead;
	strHead.Format("%s-%d-%d",packstr,theApp.CardNumi,packi);
	theApp.WriteLogThree("GetRCode(%s)",strHead);
	GetPrivateProfileString("JGPT",strHead,"2017",rCodeC,sizeof(rCodeC),theApp.logRData);
	return rCodeC;
}
...
int CCBKTcpKM2App::GetLastErrorStr(CString str)
{
	...
	if ("PWDDD"!=templ)
	{
		...
		::WritePrivateProfileString("JGPT",tempkey,tempr,logRData);
	}
	if (templ=="17Z02")
	{
		return 1;
